package com.yw.shirotemplate.shiroUtils.core.shiro.filter;
...
public class JWTFilter extends BasicHttpAuthenticationFilter {
    ...
    private boolean refreshToken(ServletRequest servletRequest, ServletResponse servletResponse) {
        ...
        String userCode = JWTUtil.getUserCode(oldToken);
        ...
        if (redisUserInfo != null) {
            if (oldToken.equals(redisUserInfo)) {
                ...
                //重写生成token(刷新)
                String newTokenStr = JWTUtil.sign(vo.getUserCode(), vo.getUserPwd());
                JWTToken jwtToken = new JWTToken(newTokenStr);
                userService.addTokenToRedis(userCode, newTokenStr);
